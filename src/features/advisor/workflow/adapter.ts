...
function resolveSchemaVersion(
  ...
  fallbackSchemaVersion: unknown,
  payloadName: string,
): string {
  if (typeof source.schema_version === 'string' && source.schema_version.length > 0) {
    return source.schema_version;
  }

  return requireString(fallbackSchemaVersion, `${payloadName}Data.schema_version`);
}

export function normalizeAppointment(raw: unknown, schemaVersion?: string): NormalizedAppointment {
    const source = raw as Record<string, unknown>;
    const session = (source.session ?? {}) as Record<string, unknown>;

    return {
        schema_version: resolveSchemaVersion(source, schemaVersion, 'appointment'),
...
export function normalizePayment(raw: unknown, schemaVersion?: string): NormalizedPayment {
    const source = raw as Record<string, unknown>;
    return {
        schema_version: resolveSchemaVersion(source, schemaVersion, 'payment'),
        appointment_id: requireString(source.appointment_id, 'appointment_id'),
...
export function normalizeTranscript(raw: unknown, schemaVersion?: string): NormalizedTranscript {
    const source = raw as Record<string, unknown>;
    return {
        schema_version: resolveSchemaVersion(source, schemaVersion, 'transcript'),
...
export function normalizeDispute(raw: unknown, schemaVersion?: string): NormalizedDispute {
    const source = raw as Record<string, unknown>;
    return {
        schema_version: resolveSchemaVersion(source, schemaVersion, 'dispute'),
